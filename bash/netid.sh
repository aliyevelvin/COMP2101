...
Address_Lan="$(hostname -I)"
Hostname_Lan=$(getent hosts $Address_Lan | awk '{print $2}')
...
function interfaces {
int=$(lshw -class network | awk '/logical name:/{print $3}' | wc -l)
for((k=1;k<=$int;k+=1));
do interface=$(lshw -class network | awk '/logical name:/{print $3}' | awk -v l=$k 'NR==l{print $1; exit}')
if [[ $interface = logic* ]];
then continue ; fi
